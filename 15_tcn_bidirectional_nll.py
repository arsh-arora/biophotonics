#!/usr/bin/env python3
"""
Bidirectional TCN with Negative Log-Likelihood (Probabilistic)
==============================================================
Probabilistic bidirectional TCN that predicts both mean and uncertainty.

Architecture:
- Bidirectional TCN backbone
- Dual output heads: mean and log-variance
- Heterosce dastic uncertainty (input-dependent)
...
- Full sequence context (bidirectional)
...
def nll_loss(pred_mean, pred_logvar, target, mask):
    """
    ...
    nll = 0.5 * (pred_logvar + squared_error / (var + 1e-6))

    # Apply mask (only missing points)
    nll_masked = nll * missing_mask
    denom = missing_mask.sum().clamp_min(1.0)

    return nll_masked.sum() / denom


class TemporalBlock(nn.Module):
    """Temporal block with symmetric padding."""
...
class BidirectionalTCN_NLL(nn.Module):
    """Bidirectional TCN with probabilistic outputs (mean + log-variance)."""
    def __init__(self, in_channels: int, out_channels: int, num_levels: int = 5,
                 ...
                 fusion: str = 'concat'):
        super(BidirectionalTCN_NLL, self).__init__()
        ...
        if fusion == 'concat':
            fusion_channels = hidden_channels * 2
        else:
            fusion_channels = hidden_channels

        # Mean head
        self.mean_head = nn.Sequential(
            nn.Conv1d(fusion_channels, hidden_channels, 1),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        Returns:
            mean: (B, C_out, T) predicted means
            logvar: (B, C_out, T) predicted log-variances
        """
        # Forward pass
        ...
        if self.fusion == 'concat':
            ...
            combined = (forward_features + backward_features) / 2.0
...
def impute_bitcn_nll(X, t, t_out, obs_mask, n_epochs=50, window=512, step=256,
                    batch_size=16, lr=1e-3, num_levels=5, hidden_channels=64,
                    dropout=0.1, fusion='concat', verbose=False):
    ...
    with torch.no_grad():
        ...
        mean_recon = x_pred_mean.transpose(1, 2).squeeze(0).cpu().numpy()
        logvar_recon = x_pred_logvar.transpose(1, 2).squeeze(0).cpu().numpy()
        var_recon = np.exp(logvar_recon)
...
def evaluate_bitcn_nll(segments, n_epochs=50, window=512, config=CONFIG, verbose=False):
    ...
    for i, seg in enumerate(segments):
        ...
        result = impute_bitcn_nll(X_true, t_seg, t_out, obs_mask,
    ...
    # NLL and calibration
    nll = 0.5 * (np.log(2 * np.pi * y_var_all) + residual**2 / y_var_all)
    nll_mean = np.mean(nll)

    z_scores = np.abs(residual) / np.sqrt(y_var_all)
...
def plot_results(t, y_true, y_pred, y_std, obs_mask, eval_mask, method_name, filename=None):
    ...
    axes[0].plot(t[eval_mask], y_pred[eval_mask], 'r-', linewidth=1.0, label='Predicted Mean', alpha=0.8)
    ...
    axes[1].set_title('Prediction Error with Uncertainty', fontsize=11, fontweight='bold')
...
def plot_zoom(t, y_true, y_pred, y_std, obs_mask, eval_mask, method_name, filename=None):
    ...
    ax.plot(t_zoom[eval_zoom], y_pred_zoom[eval_zoom], 'r-', linewidth=1.2, label='Predicted Mean', alpha=0.9)
...
def visualize_sample_result(segment, n_epochs=50, config=CONFIG):
    ...
    plot_results(t_out, y_true_1000hz, y_pred_1000hz, y_std_1000hz, obs_mask_1000, eval_mask,
                'Bidirectional TCN (NLL)', filename='15_tcn_bidirectional_nll_result.png')
    plot_zoom(t_out, y_true_1000hz, y_pred_1000hz, y_std_1000hz, obs_mask_1000, eval_mask,
             'Bidirectional TCN (NLL)', filename='15_tcn_bidirectional_nll_zoom.png')
...
if __name__ == "__main__":
    ...
    metrics = evaluate_bitcn_nll(segments, n_epochs=50, window=512,
                                config=CONFIG, verbose=False)

    print("\nGenerating visualization...")
    visualize_sample_result(segments[0], n_epochs=50, config=CONFIG)

    print("\n✓ Bidirectional TCN (NLL) evaluation complete.")
