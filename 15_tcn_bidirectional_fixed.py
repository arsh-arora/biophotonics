#!/usr/bin/env python3
"""
Bidirectional TCN (Fixed/Improved) for Trajectory Imputation
============================================================
Improved bidirectional TCN with better normalization and architecture.

Improvements over base Bi-TCN:
- Batch normalization for stable training
- Weight normalization in convolutions
- Better initialization
- Optimized hyperparameters
- Improved learning rate scheduling

Architecture:
- Forward and backward TCN branches
...
References:
- Bai, S., et al. (2018). Empirical evaluation of TCNs. arXiv:1803.01271.
...
class ImprovedTemporalBlock(nn.Module):
    """Temporal block with batch norm and weight norm."""
...
class ImprovedBidirectionalTCN(nn.Module):
    """Improved Bidirectional TCN with better normalization."""
    def __init__(self, in_channels: int, out_channels: int, num_levels: int = 5,
                 ...
                 fusion: str = 'concat'):
        super(ImprovedBidirectionalTCN, self).__init__()
        ...
        for i in range(num_levels):
            ...
            forward_layers.append(
                ImprovedTemporalBlock(num_channels, hidden_channels,
...
def impute_bitcn_fixed(X, t, t_out, obs_mask, n_epochs=50, window=512, step=256,
    ...
    model = ImprovedBidirectionalTCN(
        in_channels=D + 1,
        out_channels=D,
...
def evaluate_bitcn_fixed(segments, n_epochs=50, window=512, config=CONFIG, verbose=False):
    all_true, all_pred = [], []
    ...
    for i, seg in enumerate(segments):
        ...
        result = impute_bitcn_fixed(X_true, t_seg, t_out, obs_mask,
...
def visualize_sample_result(segment, n_epochs=50, config=CONFIG):
    ...
    plot_results(t_out, y_true_1000hz, y_pred_1000hz, obs_mask_1000, eval_mask,
                'Bidirectional TCN (Fixed)', filename='15_tcn_bidirectional_fixed_result.png')
    plot_zoom(t_out, y_true_1000hz, y_pred_1000hz, obs_mask_1000, eval_mask,
             'Bidirectional TCN (Fixed)', filename='15_tcn_bidirectional_fixed_zoom.png')
...
if __name__ == "__main__":
    ...
    metrics = evaluate_bitcn_fixed(segments, n_epochs=50, window=512,
                                  config=CONFIG, verbose=False)

    print("\nGenerating visualization...")
    visualize_sample_result(segments[0], n_epochs=50, config=CONFIG)

    print("\n✓ Bidirectional TCN (Fixed) evaluation complete.")
