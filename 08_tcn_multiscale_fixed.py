...
class MultiScaleLoss(nn.Module):
    ...
    def spectral_loss(self, y, yhat, mask):
        ...
        We *fill* missing ground-truth with the current prediction (stop-grad)
        to avoid zero-padding transients, and compute on *low-pass* versions
        to focus on drift.
        """
        # Fill missing with prediction (no gradient flows into y)
        y_filled = y * mask + yhat.detach() * (1 - mask)
        yhat_filled = yhat  # prediction stays as-is

        # Low-pass with moving average to remove HF jitter before PSD
        y_low = self.moving_average(y_filled)
        yhat_low = self.moving_average(yhat_filled)

        B, T = y.shape
        window = torch.hann_window(T, device=y.device, dtype=y.dtype)

        Y = torch.fft.rfft(y_low * window, dim=-1)
        Yhat = torch.fft.rfft(yhat_low * window, dim=-1)

        Py = (Y.abs() ** 2).mean(dim=0)
        Pyhat = (Yhat.abs() ** 2).mean(dim=0)

        # Frequency weighting: emphasize below OU corner f_c = 1/(2π τ)
        fc = 1.0 / (2.0 * math.pi * max(self.tau, 1e-6))
        n = Py.shape[0]
        # Map rfft bins to Hz assuming unit sample period scaled by fs
        freqs = torch.linspace(0.0, (n-1)/(n-1), n, device=y.device) * (self.fs / 2.0)
        w = 1.0 / (1.0 + (freqs / (fc + 1e-6))**2)  # low-pass-like weighting
        w = w / (w.mean() + 1e-12)

        return ((torch.log(Pyhat + 1e-12) - torch.log(Py + 1e-12))**2 * w).mean()
    ...
    def forward(self, y_pred, y_true, mask):
        ...
        precision_trend = torch.exp(-self.log_var_trend.clamp(-5, 5))
        precision_fast = torch.exp(-self.log_var_fast.clamp(-5, 5))
        precision_diff = torch.exp(-self.log_var_diff.clamp(-5, 5))
        precision_psd = torch.exp(-self.log_var_psd.clamp(-5, 5))
        precision_ou = torch.exp(-self.log_var_ou.clamp(-5, 5))
